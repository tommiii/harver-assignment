...
describe("CandidateMatcher", () => {
  ...
  it("handles successful file upload and matching", async () => {
    ...
    (api as jest.Mock).mockResolvedValueOnce(mockResults);
    ...
    expect(
      await screen.findByText(`Vacancy Id: ${mockResults[0].vacancyId}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`${mockResults[0].candidates[0].averageModuleScores}%`)
    ).toBeInTheDocument();
  });
  ...
  it("handles network error", async () => {
    (api as jest.Mock).mockRejectedValueOnce(
      new Error("Network error occurred while processing the request")
    );
