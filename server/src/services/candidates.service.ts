...
export class CandidatesService {
  private validateId(id: string): void {
    if (!OBJECT_ID_REGEX.test(id)) {
      ...
      throw createError(400, `Invalid 'Candidate Id': ${id} is not a valid ObjectId`);
    }
  }
  ...
  private validateModuleScore(score: string): void {
    ...
    if (isNaN(numScore)) {
      ...
      throw createError(400, `Invalid score: ${score} is not a number or 'X'`);
    }
  ...
  private validateHeader(candidatesHeaderString: string): void {
    ...
    if (candidatesHeaders[0] !== "Vacancy Id") {
      ...
      throw createError(400, `Invalid Candidate header: 'Vacancy Id' not found`);
    }

    if (candidatesHeaders[1] !== "Candidate Id") {
      logger.warn({ secondHeader: candidatesHeaders[1] }, "Missing Candidate Id header");
      throw createError(400, `Invalid Candidate header: 'Candidate Id' not found`);
    }
